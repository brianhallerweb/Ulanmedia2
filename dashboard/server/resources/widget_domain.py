...
from dashboard.server.models.widget_domain import WidgetDomainModel
...
class WidgetDomain(Resource):
    ...
    def post(self):
        ...
        if existing_widget_domain:
            return {'error_message': f'widget domain {traffic_source},{widget_id},{domain} is already in the list'}, 400

        new_widget_domain = WidgetDomainModel(traffic_source, widget_id, domain)

        try:
            new_widget_domain.save_to_db()
            return {'success_message': f'added widget domain {traffic_source},{widget_id},{domain}'} 
        except:
            return {'error_message': f'error adding widget {traffic_source},{widget_id},{domain}'}, 500
    ...
    def delete(self):
        ...
        if widget_to_be_deleted:
           try:
               ...
               return {'success_message': f'deleted widget domain {traffic_source},{widget_id},{domain}'}
           except:
               return {'error_message': f'error deleting widget domain {traffic_source},{widget_id},{domain}'}, 500
        else:
            return {'error_message': f'widget domain {traffic_source},{widget_id},{domain} does not exist in the list'}, 400 
